...
function format_playlist()
  local pl = mp.get_property_osd("playlist", "")
  local pl_new_table = {}
  local pl_new
  for i in string.gmatch(pl, "[^\r\n]+") do
    local s = "${osd-ass-cc/0}{\\fs12}${osd-ass-cc/1}"..i
    s = string.gsub(s, "{\\alpha&HFF}➜", " ")
    s = string.gsub(s, "{\\r}", "")
    table.insert(pl_new_table, s)
  end
  pl_new = table.concat(pl_new_table, "\n")
  msg.info("DEBUG pl"..utils.to_string(pl))
  msg.info("DEBUG pl_new"..pl_new)
  -- mp.osd_message(pl_new)
  mp.commandv("show-text", mp.command_native({"expand-text", pl_new}))
end

mp.add_key_binding("p", "playlist", format_playlist)
